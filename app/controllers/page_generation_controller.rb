...
class PageGenerationController < ApplicationController
  def initialize
    ...
    @code_generator = CodeGenerator.new(@openai_api_key, "Texts: "+FileReader.new(@output_path_texts).read_data["description"]+"Dominant colors: "+FileReader.new(@output_path_colors).read_data["dominantColors"])
    super
  end
  def generate_page
